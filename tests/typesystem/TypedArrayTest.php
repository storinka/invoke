...
class TypedArrayTest extends TestCase
{
    ...
    {
        $result = Typesystem::validateParam("some_items", new ArrayOf(Types::int), [1, 2, 3, 4]);
        $this->assertIsArray($result);

        $result = Typesystem::validateParam("some_items", new ArrayOf(Types::int), [1, 3.14]);
        $this->assertIsArray($result);
    }
    ...
    public function testTypedArrayAndBoolShouldFail()
    {
        ...
        Typesystem::validateParam("some_items", new ArrayOf(Types::int), [1, true]);
    }

    public function testTypedArrayAndStringShouldFail()
    {
        $this->expectException(InvalidParamTypeException::class);
        Typesystem::validateParam("some_items", new ArrayOf(Types::int), [1, "some string"]);
    }

    public function testTypedArrayAndNullShouldFail()
    {
        $this->expectException(InvalidParamTypeException::class);
        Typesystem::validateParam("some_items", new ArrayOf(Types::int), [1, null]);
    }
}
