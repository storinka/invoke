...
use Invoke\Documentation\Documents\IframeDocument;
use Invoke\Documentation\Documents\MarkdownDocument;
use Invoke\Documentation\Documents\SectionDocument;
use Invoke\Documentation\Documents\TypeDocument;
 ...
 */
class TypesSection extends SectionBuilder
{
    public function build(array $types, array $methods): SectionDocument
    {
        $types = array_values(array_filter(
            $types,
            fn(Type $type) => !($type instanceof UnionType)
        ));

        $typesDocuments = array_map(
            fn(Type $type) => TypeDocument::fromType($type),
            $types
        );

        usort($typesDocuments, function (TypeDocument $a, TypeDocument $b) {
            if ($a->name == $b->name) {
                return 0;
            }
